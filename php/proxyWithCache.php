...
if (isset($_REQUEST['headers'])) {
    $headersArray = explode(',', $_REQUEST['headers']);
    foreach ($headersArray as $key => $value) {
        $request_headers[] = urldecode($value);
    }
}
...
if (is_array($request_params) && array_key_exists('requestMethod', $request_params)) {
    $request_method = $request_params['requestMethod'];
    unset($request_params['requestMethod']);
}
...
{
    echo '<HTML><TITLE>ERROR</TITLE><BODY>Error: ' . $response_content . '</BODY></HTML>';
    die(0);
}
// (re-)send the headers
