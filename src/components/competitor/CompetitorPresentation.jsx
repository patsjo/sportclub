...
import FiveStars, { ContainerDiv, LabelDiv, StarsDiv, Loader, InfoDiv } from './FiveStars';
...
const getStarsRank = (gender, ranking) => {
  if (gender === 'MALE') {
...
const getStarsImportant = (allResults) => {
  if (!Array.isArray(allResults) || allResults.length < 2) return -1;

  let level1Ranking = allResults.filter((r) => r.ranking != null && r.levelRanking === 0);
  let level2Ranking = allResults.filter((r) => r.ranking != null && r.levelRanking === 10);
  if (level1Ranking.length === 0 && level2Ranking.length > 0) {
    level1Ranking = allResults.filter((r) => r.ranking != null && r.eventClassificationId === 'E');
    level2Ranking = allResults.filter((r) => r.ranking != null && r.eventClassificationId === 'F');
  }
  const level3Ranking = allResults.filter((r) => r.ranking != null && r.levelRanking === 20);
...
const getStarsStability = (allResults) => {
  if (!Array.isArray(allResults) || allResults.length < 5) return -1;

  const level1Ranking = allResults.filter((r) => r.ranking != null && r.levelRanking === 0);
  const level2Ranking = allResults.filter((r) => r.ranking != null && r.levelRanking === 10);
  const level3Ranking = allResults.filter((r) => r.ranking != null && r.levelRanking === 20);
...
const getStarsSpeed = (allResults, gender) => {
  if (!Array.isArray(allResults) || allResults.length === 0) return -1;
  let topSpeedRanking = allResults
...
const getStarsTechnical = (allResults, gender) => {
  if (!Array.isArray(allResults) || allResults.length === 0) return -1;
  let topTechnicalRanking = allResults
...
const getStarsRelay = (results, teamResults) => {
...
const getStarsNight = (allResults) => {
  if (!Array.isArray(allResults) || allResults.length === 0) return -1;
  ...
  const topDayRanking = allResults
    .filter(
      (r) =>
        r.ranking != null &&
        (r.deviantRaceLightCondition ? r.deviantRaceLightCondition : r.raceLightCondition) === 'Day'
    )
    .sort((a, b) => a.ranking - b.ranking)
    .slice(0, 10);
...
const getStarsShape = (allResults) => {
  if (!Array.isArray(allResults) || allResults.length < 5) return -1;
  ...
  const recent1Ranking = allResults
    .filter((r) => r.ranking != null && r.raceDate >= breakDate1)
    .sort((a, b) => a.ranking - b.ranking)
    .slice(0, 4);
  const recent2Ranking = allResults
    .filter((r) => r.ranking != null && r.raceDate >= breakDate2 && r.raceDate < breakDate1)
    .sort((a, b) => a.ranking - b.ranking)
    .slice(0, 4);
  const recent3Ranking = allResults
    .filter((r) => r.ranking != null && r.raceDate >= breakDate3 && r.raceDate < breakDate2)
    .sort((a, b) => a.ranking - b.ranking)
    .slice(0, 4);
  const old3Ranking = allResults
    .filter((r) => r.ranking != null && r.raceDate < breakDate3)
    .sort((a, b) => a.ranking - b.ranking)
    .slice(0, 12);
...
const getFavoriteDistance = (allResults) => {
  ...
  const favorite = distances.sort((a, b) => b.ranking - a.ranking)[0];

  return Math.round(favorite.lengthInMeter / (100 * favorite.nof)) * 100;
};

const CompetitorPresentation = inject(
  'clubModel',
  'sessionModel'
)(
  observer(({ clubModel, sessionModel, competitor, ranking }) => {
    const { t } = useTranslation();
    const [starsRank] = useState(getStarsRank(competitor.gender, ranking));
    const [starsImportant, setStarsImportant] = useState();
    const [starsStability, setStarsStability] = useState();
    const [starsTechnical, setStarsTechnical] = useState();
    const [starsSpeed, setStarsSpeed] = useState();
    const [starsRelay, setStarsRelay] = useState();
    const [starsNight, setStarsNight] = useState();
    const [starsShape, setStarsShape] = useState();
      ...
      PostJsonData(
        ...
        .then((c) => {
          ...
          allResults.forEach((r) => {
            r.levelRanking = ['A', 'B', 'C', 'D'].includes(r.eventClassificationId)
              ? 0
