...
const getStarsSpeed = (allResults: IAllViewResult[], gender: GenderType): TypeStars => {
  ...
  const avgSpeedRanking = topSpeedRanking.map(r => r.speedRanking!).reduce((a, b) => a + b, 0) / topSpeedRanking.length;
...
const CompetitorPresentation = observer(({ competitor, ranking }: ICompetitorPresentationProps) => {
  ...
  }, [clubModel.modules, competitor.competitorId, competitor.gender, sessionModel.authorizationHeader]);
